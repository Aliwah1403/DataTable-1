...
export function createColumnConfigHelper() {
  return {
    ...
    multiOption: () => new ColumnConfigBuilder("multiOption"),
  };
}
...
export function getColumnOptions(column, data, strategy) {
  ...
  if (column.orderFn) {
    models = models.sort((m1, m2) => column.orderFn(m1, m2));
  }
  ...
  if (column.transformOptionFn) {
    ...
    const memoizedTransform = memo(
      () => [models],
      (deps) => deps[0].map((m) => column.transformOptionFn(m)),
...
export function getColumnValues(column, data) {
  ...
  if (column.transformOptionFn) {
    const memoizedTransform = memo(
      () => [raw],
      (deps) => deps[0].map((v) => column.transformOptionFn(v)),
...
export function createColumns(data, columnConfigs, strategy) {
  return columnConfigs.map((columnConfig) => {
    ...
    const getUniqueValues = memo(
      () => [getValues(), strategy],
      ([values, strategy]) =>
        getFacetedUniqueValues(columnConfig, values, strategy),
